...
import { formatDate } from '../lib/ultility.js'
...
const NoteCard = ({note, setNotes}) => {
    ...
    const handleDelete =  async (e,id) =>{
        e.preventDefault(); //prevent user navigate to note detail when click delete icon

        if(!window.confirm("Are you sure want to delete this note?")) return

        try {
            await api.delete(`/notes/${id}`);
            toast.success("Note deleted sucsessfully")
